...
def save_classification_dataset(class_data, output_dir="classification_output", dataset_label="dataset"):
    ...
    try:
        ...
        if 'edge_embeddings' not in class_data:
...
def format_rgcn_data_for_study(dfs, 
                               ...
                               bert_model_save_dir="models/transgnn_bert"):
    """
    ...
    - REMOVED: IS_A hierarchical connections (only direct connections now)
    - STANDARDIZED: All feature connections use HAS_FEATURE relation
    ...
    print(f"    📝 NOTE: IS_A connections removed, only direct connections included")
    print(f"    📝 NOTE: All feature connections standardized to HAS_FEATURE")
    ...
    # Add all functions and features from the sample (NO PARENTS - only direct connections)
    for entry in triplets_for_study:
        ...
        # Add ONLY direct Function nodes (no parents)
        for fid, parents_ids in entry.get("functions", []):
            nodes_in_sample.add(f"Func_{fid}")
            # REMOVED: Parent function nodes are no longer added

        # Add ONLY direct Feature nodes (no parents)
        for fid, parents_ids in entry.get("features", []):
            nodes_in_sample.add(f"Feat_{str(fid)}")
            # REMOVED: Parent feature nodes are no longer added
    ...
    # REMOVED: IS_A relation is no longer used
    ...
    for entry in triplets_for_study:
        ...
        # Ceramic -> Function (DIRECT ONLY, NO HIERARCHY)
        for func_id, parents_ids in entry.get("functions", []):
            ...
            if func_idx is not None:
                ...
                # REMOVED: IS_A hierarchical links for functions
        
        # Ceramic -> Feature (DIRECT ONLY, NO HIERARCHY, STANDARDIZED TO HAS_FEATURE)
        for feat_id, parents_ids in entry.get("features", []):
            feat_id_str = str(feat_id)
            feat_idx = node_to_idx.get(f"Feat_{feat_id_str}")
            if feat_idx is not None:
                # MODIFIED: Always use HAS_FEATURE relation, ignore 'Nom complet' column
                training_triplets.append((ceramic_idx, HAS_FEATURE_REL, feat_idx))
                # REMOVED: IS_A hierarchical links for features
    ...
    for root_id in category_hierarchy.roots:
        ...
        # Root -> Features (DIRECT ONLY, STANDARDIZED TO HAS_FEATURE)
    ...
    # BERT model saving with error handling
    bert_model_info = {
    ...
    if save_bert_model:
        try:
            ...
            save_path = os.path.join(bert_model_save_dir, study_name)
            os.makedirs(save_path, exist_ok=True)
            
            print(f"    💾 Attempting to save BERT model to: {save_path}")
            
            # Method 1: Try saving with safe_serialization=False to avoid safetensors issues
            try:
                bert_model.save(save_path, safe_serialization=False)
                bert_model_info["saved"] = True
                print(f"    ✅ Successfully saved BERT model (PyTorch format)")
            except Exception as e1:
                print(f"    ⚠️ Failed to save with PyTorch format: {e1}")
                
                # Method 2: Try manual save of just the embeddings
                try:
                    # Save the embeddings manually
                    ...
                    with open(model_info_path, 'w') as f:
                        json.dump(bert_model_info, f, indent=2)
                    
                    bert_model_info["saved"] = True
                    bert_model_info["save_type"] = "embeddings_only"
                    print(f"    ✅ Saved embeddings and model info instead")
                except Exception as e2:
                    print(f"    ❌ Failed to save embeddings: {e2}")
                    bert_model_info["saved"] = False
                    bert_model_info["error"] = str(e2)
                    
        except Exception as e:
            print(f"    ❌ Error during model saving: {e}")
            bert_model_info["saved"] = False
            bert_model_info["error"] = str(e)
    ...
    rgcn_data = {
        ...
        "stats": { 
            ...
            "modifications": {
                "removed_is_a_connections": True,
                "standardized_feature_relations": "HAS_FEATURE",
    ...
    print(f"    📊 Relations in graph: {list(rgcn_data['relation_to_idx'].keys())}")
...
def prepare_three_level_studies(dfs, 
                                bert_model_name="all-MiniLM-L6-v2", 
                                auto_save=True, 
                                base_output_dir="output/transgnn_data/Bert_without_ontology",
                                save_bert_model=True, 
                                bert_model_save_dir="models/transgnn_bert"):
    """
