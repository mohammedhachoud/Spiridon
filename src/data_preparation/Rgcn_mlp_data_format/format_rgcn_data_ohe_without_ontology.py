...
def _generate_ceramic_attribute_embeddings(df):
    """Generate attribute-based embeddings for ceramic nodes - QUICK FIX"""
    print("    Generating ceramic attribute embeddings...")
    
    # Make a copy to avoid modifying original
    df_copy = df.copy()
    
    origin_values = sorted(df_copy['origin'].dropna().unique())
    all_colors = set()
    for color_list in df_copy['color_name_list'].dropna():
        try: 
    ...
    context_values = sorted(df_copy['context_type_name'].dropna().unique())
    source_values = sorted(df_copy['identifier_origin_source_name'].dropna().unique())
    ...
    # FIX: Store embeddings as object dtype to avoid the sequence error
    embeddings = []
    for index, row in df_copy.iterrows():
        embedding = get_ceramic_embedding(row)
        embeddings.append(embedding)
    
    # Store as object dtype to avoid numpy trying to create a 2D array
    df_copy['ceramic_attribute_embedding'] = pd.Series(embeddings, index=df_copy.index, dtype=object)
...
def format_rgcn_data_for_study(dfs, 
    ...
    # DEBUG: Track feature IDs that appear in sample vs. vocabulary
    sample_feature_ids = set()
    ...
    for entry in triplets_for_study:
        ...
        for fid, parents_ids in entry.get("features", []):
            ...
            sample_feature_ids.add(fid_str)  # DEBUG
            for pid in parents_ids:
                pid_str = str(pid)
                nodes_in_sample.add(f"Feat_{pid_str}")
                feature_ids_in_sample.add(pid_str)
                sample_feature_ids.add(pid_str)  # DEBUG
    ...
    # DEBUG: Check for feature ID mismatches
    print(f"    🔍 DEBUG: Feature ID vocabulary check:")
    print(f"      Sample feature IDs: {len(sample_feature_ids)}")
    print(f"      Vocabulary feature IDs: {len(all_feature_ids)}")
    
    # Find missing features
    missing_from_vocab = sample_feature_ids - set(all_feature_ids)
    if missing_from_vocab:
        print(f"      ❌ Features in sample but NOT in vocabulary: {len(missing_from_vocab)}")
        print(f"         Sample missing: {sorted(list(missing_from_vocab))[:10]}")
        
        # CRITICAL FIX: Add missing features to vocabulary
        print("      🔧 FIXING: Adding missing features to vocabulary...")
        for missing_fid in missing_from_vocab:
            all_feature_ids.append(missing_fid)
        
        # Re-sort and recreate mapping
        all_feature_ids = sorted(all_feature_ids)
        feature_id_to_pos = {fid: i for i, fid in enumerate(all_feature_ids)}
        print(f"      ✅ Updated feature vocabulary size: {len(all_feature_ids)}")
    ...
    try:
        ...
        print(f"    🔍 Ceramic embedding generation results:")
        print(f"      - ceramic_embed_info type: {type(ceramic_embed_info)}")
        print(f"      - ceramic_maps type: {type(ceramic_maps)}")
        print(f"      - ceramic_maps value: {ceramic_maps}")
        
        # Validate ceramic_maps
        if ceramic_maps is None:
            print(f"    ⚠️  Warning: ceramic_maps is None, creating empty dict")
            ceramic_maps = {}
        elif isinstance(ceramic_maps, str) and ceramic_maps == 'unknown':
            print(f"    ⚠️  Warning: ceramic_maps is 'unknown', creating empty dict")
            ceramic_maps = {}
        
        print(f"    ✅ Final ceramic_maps type: {type(ceramic_maps)}, length: {len(ceramic_maps) if hasattr(ceramic_maps, '__len__') else 'N/A'}")
        
    except Exception as e:
        print(f"    ❌ ERROR in ceramic embedding generation: {e}")
        print(f"    🔧 Using fallback values...")
        ceramic_embed_info = {'total_dim': 0}
        ceramic_attr_dim = 0
        ceramic_maps = {}
        # You might want to create a simple embedding fallback here
        ceramic_summary_with_embeddings = ceramic_summary.copy()
        ceramic_summary_with_embeddings['ceramic_attribute_embedding'] = [np.zeros(1)] * len(ceramic_summary)
    ...
    print("    🔗 Processing triplets with simplified relations...")
    training_triplets = []
    evaluation_triplets = []
    relation_to_idx = {}
    rel_idx_counter = 0
    ...
    # Note: IS_A relation is removed - no longer used
    ...
    for entry in triplets_for_study:
        ...
        # Ceramic -> Function (NO hierarchy relations)
    ...
    for root_id in category_hierarchy.roots:
        ...
        # Root -> Features (using unified HAS_FEATURE)
    ...
    rgcn_data = {
        ...
        "stats": { 
            ...
            "ceramic_attribute_maps": ceramic_maps,
            # SIMPLIFIED RELATIONS INFO
            "relation_types": list(relation_to_idx.keys()),
            "simplified_relations": True,
            "removed_isa_relations": True,
            "unified_feature_relations": True
        }
    }
    
    print(f"    ✅ Formatted data for {study_name} with SIMPLIFIED RELATIONS.")
    print(f"      🔗 Relations used: {list(relation_to_idx.keys())}")
...
def prepare_three_level_studies_onehot_without_ontology(dfs, 
                                bert_model_name="all-MiniLM-L6-v2", 
                                auto_save=True, 
                                base_output_dir="output/rgcn_data/Onehot_without_ontology",
