...
def format_rgcn_data_for_study(dfs, 
    ...
    category linking and fixed DataFrame indexing.
    
    Modified to remove IS_A relations for features while keeping custom ceramic-to-feature relations.
    """
    ...
    # Create working copies of DataFrames
    ...
    # Standardize types and handle duplicates in ceramic_summary
    ...
    # IMPORTANT: Handle duplicate ceramic_ids by keeping the first occurrence
    # This prevents the Series ambiguity error when using .loc[]
    ...
    # --- Identify all required nodes for the graph ---
    ...
    for entry in triplets_for_study:
        # Add Ceramic node
        try:
            ceramic_id = int(float(entry.get('ceramic_id')))
            nodes_in_sample.add(f"Ceramic_{ceramic_id}")
        ...
        # Add Feature nodes (but NOT their parents since we're removing IS_A relations)
        for fid, parents_ids in entry.get("features", []):
            nodes_in_sample.add(f"Feat_{str(fid)}")
            # REMOVED: Adding parent feature nodes since we don't want IS_A relations
            # for pid in parents_ids:
    ...
    # Add ALL ROOT categories to the graph
    ...
    # --- Indexing and Embedding Generation with Fixed DataFrame Access ---
    node_to_idx = {}
    idx_counter = 0
        ...
        if node_identifier not in node_to_idx:
            ...
            try:
                if node_identifier.startswith("Ceramic_"):
                    # Use dictionary lookup instead of DataFrame .loc[]
                    ...
                    # Category embedding logic (unchanged)
                ...
                # Only print warning for unexpected errors, not missing data
                if not isinstance(e, KeyError):
                    print(f"      Warning: Error processing node {node_identifier}: {e}")
            except Exception as e:
                print(f"      ERROR processing node {node_identifier}: {e}")
    
    # --- Triplet Generation (modified to remove feature IS_A relations) ---
    ...
    feature_id_to_data = Features_Ontology_dedup.set_index('id').to_dict('index')
    ...
    for entry in triplets_for_study:
        ...
        # Ceramic -> Function & Function IS_A Hierarchy (unchanged)
        for func_id, parents_ids in entry.get("functions", []):
            ...
            if func_idx is not None:
                ...
                # Add IS-A links for functions (keeping this)
                #child_idx = func_idx
        ...
        # Ceramic -> Feature (MODIFIED: Remove IS_A hierarchy for features)
        for feat_id, parents_ids in entry.get("features", []):
            ...
            if feat_idx is not None:
                ...
                if pd.isna(relation_name) or not str(relation_name).strip(): 
                    relation_name = 'HAS_FEATURE'
                rel_idx = get_or_assign_relation_idx(relation_name)
                training_triplets.append((ceramic_idx, rel_idx, feat_idx))
                
                # REMOVED: IS_A links for features - no longer adding parent-child relationships
    ...
    # Category to Function/Feature links (unchanged)
    ...
    # Final RGCN Data Dictionary Construction (unchanged from original)
    ...
    print(f"    🚫 Feature IS_A relations have been removed from the graph.")

    return rgcn_data
def prepare_three_level_studies(dfs, 
                                bert_model_name="all-MiniLM-L6-v2", 
                                auto_save=True, 
                                base_output_dir="output/rgcn_data/Bert_semi_ontology",
                                save_bert_model=True, 
                                bert_model_save_dir="models/bert"):
    """
...
def validate_duplication_integrity(ceramic_ids, ceramic_duplication_map, rgcn_data):
    """
    Validate that duplication was performed correctly.
    """
    print("\n🔍 Validating duplication integrity...")
    
    # Check 1: All ceramics in ceramic_ids should have nodes in the graph
    ceramic_nodes_in_graph = set()
    for node_id in rgcn_data['node_to_idx'].keys():
        if node_id.startswith("Ceramic_"):
            ceramic_id = int(node_id.split('_')[1])
            ceramic_nodes_in_graph.add(ceramic_id)
    
    missing_from_graph = set(ceramic_ids) - ceramic_nodes_in_graph
    extra_in_graph = ceramic_nodes_in_graph - set(ceramic_ids)
    
    print(f"  Ceramic IDs in input: {len(ceramic_ids)}")
    print(f"  Ceramic nodes in graph: {len(ceramic_nodes_in_graph)}")
    print(f"  Missing from graph: {len(missing_from_graph)}")
    print(f"  Extra in graph: {len(extra_in_graph)}")
    
    # Check 2: All ceramics should ideally have evaluation triplets
    ceramic_indices_in_eval = set()
    for head, rel, tail in rgcn_data['evaluation_triplets']:
        head_node = rgcn_data['idx_to_node'][head]
        if head_node.startswith("Ceramic_"):
            ceramic_id = int(head_node.split('_')[1])
            ceramic_indices_in_eval.add(ceramic_id)
    
    ceramics_without_eval = ceramic_nodes_in_graph - ceramic_indices_in_eval
    
    print(f"  Ceramics with evaluation triplets: {len(ceramic_indices_in_eval)}")
    print(f"  Ceramics without evaluation triplets: {len(ceramics_without_eval)}")
    
    if missing_from_graph:
        print(f"  ⚠️  Sample missing ceramics: {list(missing_from_graph)[:5]}")
    
    if ceramics_without_eval:
        print(f"  ⚠️  Sample ceramics without labels: {list(ceramics_without_eval)[:5]}")
    
    # Check 3: Verify duplication worked correctly
    original_ceramics = set(ceramic_duplication_map.keys())
    duplicated_ceramics = set()
    for duplicates in ceramic_duplication_map.values():
        duplicated_ceramics.update(duplicates)
    duplicated_ceramics -= original_ceramics
    
    print(f"  Original ceramics: {len(original_ceramics)}")
    print(f"  Duplicated ceramics: {len(duplicated_ceramics)}")
    print(f"  Expected total: {len(original_ceramics) + len(duplicated_ceramics)}")
    
    return {
        'missing_from_graph': missing_from_graph,
        'ceramics_without_eval': ceramics_without_eval,
        'coverage_ratio': len(ceramic_indices_in_eval) / len(ceramic_nodes_in_graph) if ceramic_nodes_in_graph else 0
    }
